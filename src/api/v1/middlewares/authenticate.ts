...
export const authenticateMasterToken = (
  ...
  if (!secret) {
      console.error('MASTER_TOKEN_SECRET is not set.');
      return res.status(500).send('Internal Server Error | MASTER_TOKEN_SECRET is not set.');
  }

  if(!authHeader) {
    return errorFormattedResponse(res, 403, 'No token provided.', "x-auth-token header is missing.")
  }

  if(typeof authHeader !== 'string') {
    return errorFormattedResponse(res, 403, 'Invalid token.', "x-auth-token header is invalid.")
  }
...
function validateMasterToken(authHeader: string | undefined, secret: string | undefined): boolean {
